...
export default function Table<T>({className, cols, rows, footerCols, styleMap}: Props<T>) {
    ...
    return (
        <table className={classList(css.table, className as string)} aria-label="Table">
            <thead className={css.header}>
                <ColHeaders cols={cols} styleMap={styleMap} />
            </thead>

            <tbody>
                {rows.map((row, idx) => <Row key={idx} idx={idx} row={row} cols={cols} styleMap={styleMap as TableStyleMap} />)}
            </tbody>
...
function Row<T>({idx, row, cols, styleMap}: RowProps<T>) {
    return (
        <tr className={css.row}>
            {cols.map(col => {
                return (
                    <td key={col.id} className={classList(css.cell, alignClass(col), styleMap.cells[col.id])}>
                        {col.render(row, idx)}
                    </td>
                );
            })}
